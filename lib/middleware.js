...
export default function remoteResourceMiddleware(_conf) {
  ...
  return store => next => action => {
    ...
    function reportError(error) {
      const failAction = callOpts.lifecycle.failure || defaultFailure;
    ...
    // TODO: if any of the lifecycle hooks are not there we should just use a
    // noop function for that lifecycle event
    if (
    ...
    ) return reportError(new CallProcessingError(
      'Must include URI and lifecycle hooks', 'argValidation'
    ));
    ...
    callOpts.lifecycle = Object.assign({
      request: () => null,
      success: () => null,
      failure: error => next({ type: '@@REMOTE_RESOURCE_ERROR', error })
    }, action[RemoteResource].lifecycle || {});
    ...
    // TODO: a lot of the stuff that goes in this block can definitely be
    // pulled into a requestBuilder step
    .then(([ headers, body ]) => {
      const request = {
        headers, body,
        uri: callOpts.uri,
      ...
      const statusActions = Object.keys(conf.statusActions).map(code => {
        const val = conf.statusActions[code];

        if (typeof val === 'function')
          return { code, callback: res => val(next, res) };
        else if (typeof val === 'object')
          return { code, callback: () => next(val) };
        return { code, callback: () => next({ type: val }) };
      }).reduce((acc, entry) => {
      ...
      const redirectHook = (hookVal, cb) => {
        if (typeof hookVal === 'function') return cb(hookVal);
          ...
          return () => next(hookVal);
        return () => next({ type: hookVal });
      };

      const hooks = {
        onBeforeCall: redirectHook(
          callOpts.lifecycle.request, fn => () => fn(next)
        ),
        ...
        onCallFailure: redirectHook(
          callOpts.lifecycle.failure, fn => (status, data, res) => {
            const msg = data.error || data.err || res.statusText;
            fn(new APIError(status, msg, res), next, data, res);
          }
        )
      };
