...
export default function remoteResourceMiddleware(_conf) {
  ...
  return store => next => action => {
    ...
    // TODO: don't rely on hasOwnProperty (but use it if we can)
    if (action && action.hasOwnProperty && !action.hasOwnProperty(RemoteResource))
      return next(action);
    ...
    .then(([ cachedValue, headers, body, statusActions, hooks ]) => {
      ...
      // TODO: might some clients stop working if suddenly the success hook
      // doesn't receive a response object?
