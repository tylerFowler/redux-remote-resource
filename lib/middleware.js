...
export default function remoteResourceMiddleware(_conf) {
  ...
  return store => next => action => {
    ...
    .then(([ headers, body ]) => {
      ...
      // NOTE: if this is a val we actually should still call the cb on a fn
      // that adds keys to the dispatched action
      // - Failure injected keys: `error` NOTE: doesn't comply w/ Flux SA...
      // - Success injected keys: `data`
