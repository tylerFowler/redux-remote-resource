...
/**
  ...
  @TODO allow `cacheMapping` to return a value that will be supplied to the
    ...
    short circuits
**/
export function cacheLookup(state, {
  method, cacheMapping, nocache, emitSuccessOnCacheHit
}) {
  ...
  try {
    ...
    .then(foundValue => {
      if (emitSuccessOnCacheHit) return foundValue;

      // TODO: use a Symbol here in case users want to actually use false here
      return false;
    });
  } catch (error) {
